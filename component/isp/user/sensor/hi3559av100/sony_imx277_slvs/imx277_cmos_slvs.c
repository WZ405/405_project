...
{
    {72000000, /*273*/268,  600600, 2200, 120, "4K2K_120FPS_RAW10_SLVS_8LANE"}, /*Actually 119.88fps*/
...
HI_S32 g_Imx277SlaveBindDev[ISP_MAX_PIPE_NUM] = {0, 0, 1, 1, 2, 2};
ISP_SLAVE_SNS_SYNC_S        gstImx277Sync[ISP_MAX_PIPE_NUM];
ISP_SNS_STATE_S             g_astImx277Slvs[ISP_MAX_PIPE_NUM] = {{0}};
...
ISP_SNS_COMMBUS_U g_aunImx277SlvsBusInfo[ISP_MAX_PIPE_NUM] =
{
...
#define IMX277_IS_8M120FPS_LINER_10BIT(w, h, m)    ((((3840 == w) && (2160 == h)) || ((4200 == w) && (2180 == h))) && (0 == m))
#define IMX277_IS_8M60FPS_LINER_12BIT(w, h, m)     ((((3840 == w) && (2160 == h)) || ((3936 == w) && (2182 == h))) && (2 == m))
#define IMX277_IS_8M30FPS_LINER_12BIT(w, h, m)     ((((3840 == w) && (2160 == h)) || ((3936 == w) && (2182 == h))) && (1 == m))
#define IMX277_IS_12M30FPS_LINER_12BIT(w, h, m)    ((((4000 == w) && (3000 == h))|| ((4104 == w) && (3062 == h))) && (0 == m))
#define IMX277_IS_2M240FPS_LINER_12BIT(w, h, m)    ((((1920 == w) && (1080 == h))|| ((1968 == w) && (1102 == h))) && (0 == m))
...
static HI_S32 cmos_get_ae_default(VI_PIPE ViPipe, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    u32Fll = g_astImx277ModeTbl[g_apstSnsState[ViPipe]->u8ImgMode].u32VertiLines;
    U32MaxFps = g_astImx277ModeTbl[g_apstSnsState[ViPipe]->u8ImgMode].u32MaxFps;
    g_apstSnsState[ViPipe]->u32FLStd = u32Fll;

    memset(&pstAeSnsDft->stAERouteAttr, 0, sizeof(ISP_AE_ROUTE_S));

	pstAeSnsDft->u32InitExposure = 921600;
    pstAeSnsDft->u32LinesPer500ms = (g_apstSnsState[ViPipe]->u32FLStd * U32MaxFps) >> 1;
...
static HI_VOID cmos_fps_set(VI_PIPE ViPipe, HI_FLOAT f32Fps, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    u32MaxFps = g_astImx277ModeTbl[g_apstSnsState[ViPipe]->u8ImgMode].u32MaxFps;
    u32Lines = g_astImx277ModeTbl[g_apstSnsState[ViPipe]->u8ImgMode].u32VertiLines * u32MaxFps / f32Fps;
    ...
    gstImx277Sync[ViPipe].u32VsTime = (g_astImx277ModeTbl[g_apstSnsState[ViPipe]->u8ImgMode].u32InckPerVs) * u32MaxFps / f32Fps;
    g_apstSnsState[ViPipe]->u32FLStd = u32Lines;
    g_apstSnsState[ViPipe]->astRegsInfo[0].stSlvSync.u32SlaveVsTime = gstImx277Sync[ViPipe].u32VsTime;
                                       ...
                                       AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    pstAeSnsDft->u32FullLines = g_apstSnsState[ViPipe]->au32FL[0];
    gstImx277Sync[ViPipe].u32VsTime = g_astImx277ModeTbl[g_apstSnsState[ViPipe]->u8ImgMode].u32InckPerHs * u32FullLines;
    g_apstSnsState[ViPipe]->astRegsInfo[0].stSlvSync.u32SlaveVsTime = gstImx277Sync[ViPipe].u32VsTime;
...
static HI_VOID cmos_inttime_update(VI_PIPE ViPipe, HI_U32 u32IntTime)
{
    ...
    {
        u32Shr = g_apstSnsState[ViPipe]->au32FL[0] - u32IntTime - 4;
...
static HI_S32 cmos_set_wdr_mode(VI_PIPE ViPipe, HI_U8 u8Mode)
{
    ...
    {
        case WDR_MODE_NONE:
            if ((IMX277_8M120FPS_LINER_MODE == g_apstSnsState[ViPipe]->u8ImgMode) || (IMX277_8M30FPS_LINER_MODE == g_apstSnsState[ViPipe]->u8ImgMode)
                || (IMX277_8M60FPS_LINER_MODE == g_apstSnsState[ViPipe]->u8ImgMode))
            {
                g_apstSnsState[ViPipe]->u32FLStd = 2200;
            }
            else if (IMX277_12M30FPS_LINER_MODE == g_apstSnsState[ViPipe]->u8ImgMode)
            {
                g_apstSnsState[ViPipe]->u32FLStd = 3080;
            }
            else if (IMX277_2M240FPS_LINER_MODE == g_apstSnsState[ViPipe]->u8ImgMode)
            {
                g_apstSnsState[ViPipe]->u32FLStd = 1118;
            }
            ...
            break;
        case WDR_MODE_2To1_LINE:
            if (IMX277_8M60FPS_WDR_MODE == g_apstSnsState[ViPipe]->u8ImgMode)
            {
                g_apstSnsState[ViPipe]->u32FLStd = g_astImx277ModeTbl[g_apstSnsState[ViPipe]->u8ImgMode].u32VertiLines;
            }
...
static HI_S32 cmos_get_sns_regs_info(VI_PIPE ViPipe, ISP_SNS_REGS_INFO_S *pstSnsRegsInfo)
{
    ...
    if ((HI_FALSE == g_apstSnsState[ViPipe]->bSyncInit) || (HI_FALSE == pstSnsRegsInfo->bConfig))
    {
        ...
        g_apstSnsState[ViPipe]->astRegsInfo[0].unComBus.s8SspDev.bit4SspDev = g_aunImx277SlvsBusInfo[ViPipe].s8SspDev.bit4SspDev;
        g_apstSnsState[ViPipe]->astRegsInfo[0].unComBus.s8SspDev.bit4SspCs = g_aunImx277SlvsBusInfo[ViPipe].s8SspDev.bit4SspCs;
        ...
        g_apstSnsState[ViPipe]->astRegsInfo[0].astSspData[0].u32RegAddr = IMX277_SHR_ADDR_H;
        g_apstSnsState[ViPipe]->astRegsInfo[0].astSspData[1].u8DelayFrmNum = 0;
        g_apstSnsState[ViPipe]->astRegsInfo[0].astSspData[1].u32RegAddr = IMX277_SHR_ADDR_L;

        /* AG */
        g_apstSnsState[ViPipe]->astRegsInfo[0].astSspData[2].u8DelayFrmNum = 1;
        g_apstSnsState[ViPipe]->astRegsInfo[0].astSspData[2].u32RegAddr = IMX277_PGC_ADDR_L;
        g_apstSnsState[ViPipe]->astRegsInfo[0].astSspData[3].u8DelayFrmNum = 1;
        g_apstSnsState[ViPipe]->astRegsInfo[0].astSspData[3].u32RegAddr = IMX277_PGC_ADDR_H;

        /* DG */
        g_apstSnsState[ViPipe]->astRegsInfo[0].astSspData[4].u8DelayFrmNum = 1;
        g_apstSnsState[ViPipe]->astRegsInfo[0].astSspData[4].u32RegAddr = IMX277_DGAIN_ADDR;

        //DOL 2t1 Mode Regs
        if (WDR_MODE_2To1_LINE == g_apstSnsState[ViPipe]->enWDRMode)
        {
            g_apstSnsState[ViPipe]->astRegsInfo[0].astSspData[5].u8DelayFrmNum = 0;
            g_apstSnsState[ViPipe]->astRegsInfo[0].astSspData[5].u32RegAddr = IMX277_SHR_SE_ADDR_H;
            g_apstSnsState[ViPipe]->astRegsInfo[0].astSspData[6].u8DelayFrmNum = 0;
            g_apstSnsState[ViPipe]->astRegsInfo[0].astSspData[6].u32RegAddr = IMX277_SHR_SE_ADDR_L;

            g_apstSnsState[ViPipe]->astRegsInfo[0].astSspData[7].u8DelayFrmNum = 1;
            g_apstSnsState[ViPipe]->astRegsInfo[0].astSspData[7].u32RegAddr = IMX277_PGC_SE_ADDR_L;
            g_apstSnsState[ViPipe]->astRegsInfo[0].astSspData[8].u8DelayFrmNum = 1;
            g_apstSnsState[ViPipe]->astRegsInfo[0].astSspData[8].u32RegAddr = IMX277_PGC_SE_ADDR_H;
        }
        ...
        g_apstSnsState[ViPipe]->astRegsInfo[0].stSlvSync.u32SlaveBindDev = g_Imx277SlaveBindDev[ViPipe];
...
static HI_S32 cmos_set_image_mode(VI_PIPE ViPipe, ISP_CMOS_SENSOR_IMAGE_MODE_S *pstSensorImageMode)
{
    ...
    if (WDR_MODE_2To1_LINE == g_apstSnsState[ViPipe]->enWDRMode)
    {
        //u8SensorImageMode = IMX277_8M60FPS_WDR_MODE;
        printf("IMX277_8M60FPS_WDR_MODE not support yet...\n");
...
static HI_VOID sensor_global_init(VI_PIPE ViPipe)
{
    ...
    g_apstSnsState[ViPipe]->bSyncInit = HI_FALSE;
    g_apstSnsState[ViPipe]->u8ImgMode = IMX277_8M120FPS_LINER_MODE;
    g_apstSnsState[ViPipe]->u32FLStd = 2200;
...
static int imx277_slvs_set_bus_info(VI_PIPE ViPipe, ISP_SNS_COMMBUS_U unSNSBusInfo)
{
    g_aunImx277SlvsBusInfo[ViPipe].s8SspDev.bit4SspDev = unSNSBusInfo.s8SspDev.bit4SspDev;
    g_aunImx277SlvsBusInfo[ViPipe].s8SspDev.bit4SspCs  = unSNSBusInfo.s8SspDev.bit4SspCs;
