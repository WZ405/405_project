                    ...
                    SVP_SRC_IMAGE_S *pstSrc,SVP_DST_IMAGE_S *pstDst,SVP_MEM_INFO_S *pstAssistBuf)
{
    ...
    printf("size of pstSrc %d\n",sizeof(*pstSrc));

    memcpy(pu8Tmp,pstSrc,sizeof(*pstSrc)); //copy src image from pstAssistBuf to pstSrc
    pu8Tmp += sizeof(*pstSrc);

    printf("size of pstDst %d\n",sizeof(*pstDst));

    memcpy(pu8Tmp,pstDst,sizeof(*pstDst)); //copy dst image from pstAssistBuf to pstDst
