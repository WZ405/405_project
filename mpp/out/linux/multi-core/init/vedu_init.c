...
extern void * g_pstJpgeCHIP_Reg[1];
...
extern HI_U32  vedu_en[VEDU_NUM];
module_param_array(vedu_en, uint, HI_NULL, S_IRUGO);
...
HI_VOID VEDU_GetVpuInfo(HI_U32 vpu[])
{
    HI_U32 VeduNum=0;
    HI_U32 i=0;

    for(i=0; i<VEDU_NUM; i++)
    {
        if(1 == vedu_en[i])
        {
            vpu[i] = 1;
