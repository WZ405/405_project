...
typedef struct hiPCIV_CHANNEL_S
{
    ...
    volatile PCIV_BUFF_STATUS_E aenBuffStatus[PCIV_MAX_BUF_NUM];  /* Used by sender. */
...
HI_S32 PcivCheckAttr(PCIV_ATTR_S *pAttr)
{
    ...
    {
        if(!pAttr->u64PhyAddr[i])
        {
            PCIV_TRACE(HI_DBG_ERR, "pAttr->u64PhyAddr[%d]:0x%x invalid\n", i, pAttr->u64PhyAddr[i]);
            return HI_ERR_PCIV_ILLEGAL_PARAM;
        }
    }
...
HI_S32 PCIV_Create(PCIV_CHN pcivChn, PCIV_ATTR_S *pAttr)
{
    ...
    {
        pChn->aenBuffStatus[i] = BUFF_BUSY;
    }

    for (i = 0; i < pAttr->u32Count; i++)
    {
        if (0 != pAttr->u64PhyAddr[i])
        {
            pChn->aenBuffStatus[i] = BUFF_FREE;
        }
    }
...
HI_S32 PCIV_Malloc(HI_U32 u32Size, HI_U64 *pPhyAddr)
{
    HI_S32 s32LocalId = PCIV_DrvAdp_GetLocalId();
...
HI_S32 PCIV_Free(HI_U64 u64PhyAddr)
{
	HI_S32 s32LocalId = PCIV_DrvAdp_GetLocalId();

    return PCIV_FirmWareFree(s32LocalId, u64PhyAddr);
}

HI_S32 PCIV_MallocChnBuffer(PCIV_CHN pcivChn, HI_U32 u32Index, HI_U32 u32Size, HI_U64 *pPhyAddr)
{
    HI_S32 s32LocalId = PCIV_DrvAdp_GetLocalId();
...
HI_VOID  PcivUserDmaDone(PCIV_SENDTASK_S *pstTask)
{
    ...
    HI_ASSERT((pstTask->u64PrvData[0] + 1) == pstTask->u64PrvData[1]);

    pUserDmaNode = (PCIV_USERDMANODE_S *)pstTask->u64PrvData[2];
...
HI_S32 PCIV_UserDmaTask(PCIV_DMA_TASK_S *pTask)
{
    ...
    {
        stPciTask.u64SrcPhyAddr = pTask->pBlock[i].u64SrcAddr;
        stPciTask.u64DstPhyAddr = pTask->pBlock[i].u64DstAddr;
        stPciTask.u32Len        = pTask->pBlock[i].u32BlkSize;
        stPciTask.bRead         = pTask->bRead;
        stPciTask.u64PrvData[0] = i;
        stPciTask.u64PrvData[1] = pTask->u32Count;
        stPciTask.u64PrvData[2] = (HI_U64)pUserDmaNode;
...
HI_S32 PCIV_FreeShareBuf(PCIV_CHN pcivChn, HI_U32 u32Index, HI_U32 u32Count)
{
    ...
    pstPcivChn->aenBuffStatus[u32Index] = BUFF_FREE;
    PCIV_SPIN_UNLOCK;
...
HI_S32 PCIV_FreeAllBuf(PCIV_CHN pcivChn)
{
    ...
    {
        g_stPcivChn[pcivChn].aenBuffStatus[i] = BUFF_FREE;
    }
...
static HI_S32 PCIV_GetShareBuf(PCIV_CHN pcivChn, HI_U32 *pCurIndex)
{
    ...
    {
        if(BUFF_HOLD == pstPcivChn->aenBuffStatus[i])
        {
            *pCurIndex = i;
            return HI_SUCCESS;
        }
    ...
    for(i=0; i<pstPcivChn->stPcivAttr.u32Count; i++)
    {
        if(BUFF_FREE == pstPcivChn->aenBuffStatus[i])
        {
            *pCurIndex = i;
            return HI_SUCCESS;
        }
...
static HI_S32 PCIV_GetShareBufState(PCIV_CHN pcivChn)
{
    ...
    for(i=0; i<pstPcivChn->stPcivAttr.u32Count; i++)
    {
        if(BUFF_FREE == pstPcivChn->aenBuffStatus[i])
        {
            pstPcivChn->aenBuffStatus[i] = BUFF_HOLD;
            PCIV_SPIN_UNLOCK;
            return HI_SUCCESS;
        }
    }
...
HI_VOID PcivSrcPicSendDone(PCIV_SENDTASK_S *pTask)
{
    ...
    pTask->pvPrvData = NULL;
    (void)PCIV_DrvAdp_DmaEndNotify(&pstChn->stPcivAttr.stRemoteObj, &stRecvPic);

    stSrcPic.u64PhyAddr = pTask->u64SrcPhyAddr;
    stSrcPic.u32PoolId  = pTask->u64PrvData[1];
...
HI_S32 PCIV_SrcPicSend(PCIV_CHN pcivChn, PCIV_PIC_S *pSrcPic)
{
    ...
    if(HI_SUCCESS != s32Ret)
    {
        ...
        PCIV_TRACE(HI_DBG_ERR, "No Free Buf, chn%d,enSrcType:%d\n", pcivChn,pSrcPic->enSrcType);
        PCIV_SPIN_UNLOCK;
    ...
    pstRecvPic->enModId                     = pSrcPic->enModId;
    pstRecvPic->bBlock                      = pSrcPic->bBlock;
    pstRecvPic->enColorGamut                = pSrcPic->enColorGamut;
    pstRecvPic->enCompressMode              = pSrcPic->enCompressMode;
    pstRecvPic->enDynamicRange              = pSrcPic->enDynamicRange;
    pstRecvPic->enVideoFormat               = pSrcPic->enVideoFormat;
    ...
    stPciTask.u64SrcPhyAddr = pSrcPic->u64PhyAddr;
    stPciTask.u64DstPhyAddr = pChn->stPcivAttr.u64PhyAddr[u32CurIndex];
    stPciTask.bRead         = HI_FALSE;
    stPciTask.u64PrvData[0] = pcivChn;              /* channel num */
    stPciTask.u64PrvData[1] = pSrcPic->u32PoolId;   /* src Image PoolID */
    ...
    HI_ASSERT((BUFF_HOLD== pChn->aenBuffStatus[u32CurIndex])||(BUFF_FREE== pChn->aenBuffStatus[u32CurIndex]));
    pChn->aenBuffStatus[u32CurIndex] = BUFF_BUSY;
    pChn->u32SendCnt++;
...
HI_S32 PCIV_ReceivePic(PCIV_CHN pcivChn, PCIV_PIC_S *pRecvPic)
{
   ...
   /************************************************************************************
    ...
    {
    	g_stPcivChn[pcivChn].aenBuffStatus[pRecvPic->u32Index] = BUFF_BUSY;
        PCIV_SPIN_UNLOCK;
		pRecvPic->u64Pts = 0;
    ...
    HI_ASSERT(BUFF_FREE== g_stPcivChn[pcivChn].aenBuffStatus[pRecvPic->u32Index]);
...
HI_S32 PCIV_RecvPicFree(PCIV_CHN pcivChn, PCIV_PIC_S *pRecvPic)
{
    ...
    if(BUFF_BUSY== pChn->aenBuffStatus[pRecvPic->u32Index])
    {
		/*Buffer state is set to idle*/
        pChn->aenBuffStatus[pRecvPic->u32Index] = BUFF_FREE;
...
{
    if (PIXEL_FORMAT_YVU_SEMIPLANAR_420 == enPixFormt)
    {
        return "sp420";
    }
    else if (PIXEL_FORMAT_YVU_SEMIPLANAR_422 == enPixFormt)
    {
        return "sp422";
    }
    else if (PIXEL_FORMAT_YVU_PLANAR_420 == enPixFormt)
    {
        return "p420";
    }
    else if (PIXEL_FORMAT_YVU_PLANAR_422 == enPixFormt)
    {
        return "p422";
    }
...
HI_S32 PCIV_ProcShow(osal_proc_entry_t *s)
{
    ...
    for (pcivChn = 0; pcivChn < PCIV_MAX_CHN_NUM; pcivChn++)
    {
        ...
        osal_seq_printf(s, "%6d" "%8d" "%8d" "%8d" "%8s" "%8s" "%8d" "%10d" "%10x" "\n",
            ...
            pstAttr->u32Count,
            pstAttr->u32BlkSize,
            pstAttr->u64PhyAddr[0]);
    }

    osal_seq_printf(s, "\n-----PCIV CHN STATUS------------------------------------------------------------\n");
    osal_seq_printf(s, "%6s"    "%8s"     "%8s"     "%8s"    "%8s"     "%8s"     "%8s"     "%8s"     "%12s"    "\n"
                 ,"PciChn","RemtChp","RemtChn","GetCnt","SendCnt","RespCnt","LostCnt","NtfyCnt","BuffStatus");
    for (pcivChn = 0; pcivChn < PCIV_MAX_CHN_NUM; pcivChn++)
    {
        pstPcivChn = &g_stPcivChn[pcivChn];
        if (HI_FALSE == pstPcivChn->bCreate) continue;
        ...
        for(i=0; i<pstPcivChn->stPcivAttr.u32Count; i++)
        {
            sprintf(&cString[i*2], "%2d", pstPcivChn->aenBuffStatus[i]);
        }
