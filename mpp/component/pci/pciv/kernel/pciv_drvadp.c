...
HI_VOID PcivDrvAdpStartDmaTask(HI_VOID)
{
    ...
    while(!list_empty(&(g_listDmaTask)))
    {
        ...
        pciTask.src          = pTask->u64SrcPhyAddr;
        pciTask.dest         = pTask->u64DstPhyAddr;
...
HI_S32 PCIV_DrvAdp_Init(void)
{
    ...
    if(g_s32LocalId == 0)
    {
        ...
        for(i=0; i<HISI_MAX_MAP_DEV; i++)
        {
            if(nRemotId[i] != -1)
            {
                ...
                pBaseWin->u64PfWinBase  = get_pf_window_base(nRemotId[i]);
                pBaseWin->u64PfAHBAddr  = 0;
    ...
    else
    {
        ...
        pBaseWin->u64NpWinBase  = 0;
        pBaseWin->u64PfWinBase  = 0;
        pBaseWin->u64CfgWinBase = 0;
        pBaseWin->u64PfAHBAddr  = get_pf_window_base(0);
    }
