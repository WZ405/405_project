...
HI_S32  HI_MPI_PCIV_Malloc(HI_U32 u32BlkSize, HI_U32 u32BlkCnt, HI_U64 u64PhyAddr[])
{
    ...
    for(s32Cnt=0; s32Cnt<u32BlkCnt; s32Cnt++)
    {
        ...
        u64PhyAddr[s32Cnt] = stMalloc.u64PhyAddr;
    }
    ...
    if(HI_SUCCESS != s32Ret)
    {
        ...
        for( ; s32Cnt>=0; s32Cnt--)
        {
            (HI_VOID)IOCTL(g_s32PcivFd[0], PCIV_FREE_CTRL, &u64PhyAddr[s32Cnt]);
        }
    }
...
HI_S32  HI_MPI_PCIV_Free(HI_U32 u32BlkCnt, HI_U64 u64PhyAddr[])
{
    ...
    {
        s32Ret = IOCTL(g_s32PcivFd[0], PCIV_FREE_CTRL, &u64PhyAddr[i]);
		if (HI_SUCCESS != s32Ret)
		{
			break;
		}
