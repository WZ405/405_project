...
ISP_SNS_STATE_S  g_astImx334[ISP_MAX_PIPE_NUM] = {{0}};
...
#define IMX334_FULL_LINES_MAX_2TO1_WDR  (0xFFFFF)  //considering the YOUT_SIZE and bad frame
...
static HI_VOID cmos_fps_set(VI_PIPE ViPipe, HI_FLOAT f32Fps, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    switch (g_apstSnsState[ViPipe]->u8ImgMode)
    {
        case IMX334_8M_30FPS_12BIT_LINEAR_MODE:
            ...
            {
                u32Lines = IMX334_VMAX_8M_30FPS_12BIT_LINEAR * u32MaxFps / f32Fps;
            }
            else
            {
        ...
        case IMX334_8M_30FPS_12BIT_2t1_DOL_MODE:
            ...
            {
                u32Lines = IMX334_VMAX_8M_30FPS_12BIT_2TO1_WDR * u32MaxFps / f32Fps;
            }
            else
            {
                printf("Not support Fps: %f\n", f32Fps);
    ...
    if (WDR_MODE_2To1_LINE == g_apstSnsState[ViPipe]->enWDRMode)
    {
        ...
        else
        {
            ...
            else if (u32RHS1[ViPipe] < u32LastRHS1[ViPipe])
            {
                ...
                {
                    u32SHR1[ViPipe] = 9 - (u32deltaRHS1[ViPipe] - g_astimx334State[ViPipe].u32deltaRHS1) % 4
                                      + (u32deltaRHS1[ViPipe] - g_astimx334State[ViPipe].u32deltaRHS1);
                    u32RHS1[ViPipe] = u32LastRHS1[ViPipe] - (g_astimx334State[ViPipe].u32deltaRHS1 - g_astimx334State[ViPipe].u32deltaRHS1 % 4);
                }
...
static HI_VOID cmos_get_inttime_max(VI_PIPE ViPipe, HI_U16 u16ManRatioEnable, HI_U32 *au32Ratio, HI_U32 *au32IntTimeMax, HI_U32 *au32IntTimeMin, HI_U32 *pu32LFMaxIntTime)
{
    ...
    if (WDR_MODE_2To1_LINE == g_apstSnsState[ViPipe]->enWDRMode)
    {
        ...
        if (ISP_FSWDR_LONG_FRAME_MODE == genFSWDRMode[ViPipe])
        {
            u32IntTimeMaxTmp0 = g_apstSnsState[ViPipe]->au32FL[1] - 280 - g_apstSnsState[ViPipe]->au32WDRIntTime[0];
            u32IntTimeMaxTmp = g_apstSnsState[ViPipe]->au32FL[0] - 280;
        ...
        {
            u32IntTimeMaxTmp0 = ((g_apstSnsState[ViPipe]->au32FL[1] - 280 - g_apstSnsState[ViPipe]->au32WDRIntTime[0]) * 0x40)  / DIV_0_TO_1(au32Ratio[0]);
            u32IntTimeMaxTmp = ((g_apstSnsState[ViPipe]->au32FL[0] - 280) * 0x40)  / DIV_0_TO_1(au32Ratio[0] + 0x40);
            u32IntTimeMaxTmp = (u32IntTimeMaxTmp0 < u32IntTimeMaxTmp) ? u32IntTimeMaxTmp0 : u32IntTimeMaxTmp;
...
#if 0      //awb static param for stitching lens
#define CALIBRATE_STATIC_WB_R_GAIN 478
#define CALIBRATE_STATIC_WB_GR_GAIN 256
#define CALIBRATE_STATIC_WB_GB_GAIN 256
#define CALIBRATE_STATIC_WB_B_GAIN 503

/* Calibration results for Auto WB Planck */
#define CALIBRATE_AWB_P1 -10
#define CALIBRATE_AWB_P2 263
#define CALIBRATE_AWB_Q1 -4
#define CALIBRATE_AWB_A1 146570
#define CALIBRATE_AWB_B1 128
#define CALIBRATE_AWB_C1 -94575
#endif
...
static HI_VOID cmos_set_pixel_detect(VI_PIPE ViPipe, HI_BOOL bEnable)
{
    ...
    {
        if (IMX334_8M_30FPS_12BIT_LINEAR_MODE == g_apstSnsState[ViPipe]->u8ImgMode)
        {
    ...
    if (bEnable) /* setup for ISP pixel calibration mode */
    {
        ...
        imx334_write_register(ViPipe, IMX334_SHR0_L, u32MaxIntTime_5Fps & 0xFF);
        imx334_write_register(ViPipe, IMX334_SHR0_M, (u32MaxIntTime_5Fps & 0xFF00) >> 8);
        imx334_write_register(ViPipe, IMX334_SHR0_H, (u32MaxIntTime_5Fps & 0xF0000) >> 16);
    }
    else /* setup for ISP 'normal mode' */
    {
        g_apstSnsState[ViPipe]->u32FLStd = (g_apstSnsState[ViPipe]->u32FLStd > IMX334_FULL_LINES_MAX) ? IMX334_FULL_LINES_MAX : g_apstSnsState[ViPipe]->u32FLStd;
        imx334_write_register(ViPipe, IMX334_VMAX_L, (g_apstSnsState[ViPipe]->u32FLStd & 0xff));
        imx334_write_register(ViPipe, IMX334_VMAX_M, (g_apstSnsState[ViPipe]->u32FLStd & 0xff00) >> 8);
        imx334_write_register(ViPipe, IMX334_VMAX_H,  (g_apstSnsState[ViPipe]->u32FLStd & 0xf0000) >> 16);
...
static HI_S32 cmos_set_wdr_mode(VI_PIPE ViPipe, HI_U8 u8Mode)
{
    ...
    switch (u8Mode)
    {
        case WDR_MODE_NONE:
            ...
            g_apstSnsState[ViPipe]->u32FLStd = IMX334_VMAX_8M_30FPS_12BIT_LINEAR ;
            printf("linear mode\n");
            break;

        case WDR_MODE_2To1_LINE:
            g_apstSnsState[ViPipe]->enWDRMode = WDR_MODE_2To1_LINE;
            g_apstSnsState[ViPipe]->u32FLStd = IMX334_VMAX_8M_30FPS_12BIT_2TO1_WDR * 2;
...
static HI_S32 cmos_get_sns_regs_info(VI_PIPE ViPipe, ISP_SNS_REGS_INFO_S *pstSnsRegsInfo)
{
    ...
    if ((HI_FALSE == g_apstSnsState[ViPipe]->bSyncInit) || (HI_FALSE == pstSnsRegsInfo->bConfig))
    {
        ...
        g_apstSnsState[ViPipe]->astRegsInfo[0].unComBus.s8I2cDev = g_aunImx334BusInfo[ViPipe].s8I2cDev;
        ...
        for (i = 0; i < g_apstSnsState[ViPipe]->astRegsInfo[0].u32RegNum; i++)
        {
            ...
            g_apstSnsState[ViPipe]->astRegsInfo[0].astI2cData[i].u8DevAddr = imx334_i2c_addr;
            g_apstSnsState[ViPipe]->astRegsInfo[0].astI2cData[i].u32AddrByteNum = imx334_addr_byte;
            g_apstSnsState[ViPipe]->astRegsInfo[0].astI2cData[i].u32DataByteNum = imx334_data_byte;
        }

        //SHR0 related
        g_apstSnsState[ViPipe]->astRegsInfo[0].astI2cData[0].u8DelayFrmNum = 0;
        g_apstSnsState[ViPipe]->astRegsInfo[0].astI2cData[0].u32RegAddr = IMX334_SHR0_L;
        g_apstSnsState[ViPipe]->astRegsInfo[0].astI2cData[1].u8DelayFrmNum = 0;
        g_apstSnsState[ViPipe]->astRegsInfo[0].astI2cData[1].u32RegAddr = IMX334_SHR0_M;
        g_apstSnsState[ViPipe]->astRegsInfo[0].astI2cData[2].u8DelayFrmNum = 0;
        g_apstSnsState[ViPipe]->astRegsInfo[0].astI2cData[2].u32RegAddr = IMX334_SHR0_H;

        // gain related
        g_apstSnsState[ViPipe]->astRegsInfo[0].astI2cData[3].u32RegAddr = IMX334_PGC_L;
        g_apstSnsState[ViPipe]->astRegsInfo[0].astI2cData[3].u8DelayFrmNum = 0;
        g_apstSnsState[ViPipe]->astRegsInfo[0].astI2cData[4].u32RegAddr = IMX334_PGC_H;
        ...
        g_apstSnsState[ViPipe]->astRegsInfo[0].astI2cData[5].u32RegAddr = IMX334_VMAX_L;
        g_apstSnsState[ViPipe]->astRegsInfo[0].astI2cData[6].u8DelayFrmNum = 0;
        g_apstSnsState[ViPipe]->astRegsInfo[0].astI2cData[6].u32RegAddr = IMX334_VMAX_M;
        g_apstSnsState[ViPipe]->astRegsInfo[0].astI2cData[7].u8DelayFrmNum = 0;
        g_apstSnsState[ViPipe]->astRegsInfo[0].astI2cData[7].u32RegAddr = IMX334_VMAX_H;
        ...
        if (WDR_MODE_2To1_LINE == g_apstSnsState[ViPipe]->enWDRMode)
        {
            ...
            g_apstSnsState[ViPipe]->astRegsInfo[0].astI2cData[5].u8DelayFrmNum = 0;
            g_apstSnsState[ViPipe]->astRegsInfo[0].astI2cData[5].u32RegAddr = IMX334_SHR1_L;
            g_apstSnsState[ViPipe]->astRegsInfo[0].astI2cData[6].u8DelayFrmNum = 0;
            g_apstSnsState[ViPipe]->astRegsInfo[0].astI2cData[6].u32RegAddr = IMX334_SHR1_M;
            g_apstSnsState[ViPipe]->astRegsInfo[0].astI2cData[7].u8DelayFrmNum = 0;
            g_apstSnsState[ViPipe]->astRegsInfo[0].astI2cData[7].u32RegAddr = IMX334_SHR1_H;

            g_apstSnsState[ViPipe]->astRegsInfo[0].astI2cData[8].u8DelayFrmNum = 1;
            g_apstSnsState[ViPipe]->astRegsInfo[0].astI2cData[8].u32RegAddr = IMX334_VMAX_L;
            g_apstSnsState[ViPipe]->astRegsInfo[0].astI2cData[9].u8DelayFrmNum = 1;
            g_apstSnsState[ViPipe]->astRegsInfo[0].astI2cData[9].u32RegAddr = IMX334_VMAX_M;
            g_apstSnsState[ViPipe]->astRegsInfo[0].astI2cData[10].u8DelayFrmNum = 1;
            g_apstSnsState[ViPipe]->astRegsInfo[0].astI2cData[10].u32RegAddr = IMX334_VMAX_H;

            g_apstSnsState[ViPipe]->astRegsInfo[0].astI2cData[11].u8DelayFrmNum = 0;
            g_apstSnsState[ViPipe]->astRegsInfo[0].astI2cData[11].u32RegAddr = IMX334_RHS1_L;
            g_apstSnsState[ViPipe]->astRegsInfo[0].astI2cData[12].u8DelayFrmNum = 0;
            g_apstSnsState[ViPipe]->astRegsInfo[0].astI2cData[12].u32RegAddr = IMX334_RHS1_M;
            g_apstSnsState[ViPipe]->astRegsInfo[0].astI2cData[13].u8DelayFrmNum = 0;
            g_apstSnsState[ViPipe]->astRegsInfo[0].astI2cData[13].u32RegAddr = IMX334_RHS1_H;

            g_apstSnsState[ViPipe]->astRegsInfo[0].astI2cData[14].u8DelayFrmNum = 0;
            g_apstSnsState[ViPipe]->astRegsInfo[0].astI2cData[14].u32RegAddr = IMX334_Y_OUTSIZE_L;
            g_apstSnsState[ViPipe]->astRegsInfo[0].astI2cData[15].u8DelayFrmNum = 0;
            g_apstSnsState[ViPipe]->astRegsInfo[0].astI2cData[15].u32RegAddr = IMX334_Y_OUTSIZE_H;
...
static HI_VOID sensor_global_init(VI_PIPE ViPipe)
{
    ...
    g_apstSnsState[ViPipe]->bSyncInit = HI_FALSE;
    g_apstSnsState[ViPipe]->u8ImgMode = IMX334_8M_30FPS_12BIT_LINEAR_MODE;
    g_apstSnsState[ViPipe]->enWDRMode = WDR_MODE_NONE;
    g_apstSnsState[ViPipe]->u32FLStd = IMX334_VMAX_8M_30FPS_12BIT_LINEAR;
    g_apstSnsState[ViPipe]->au32FL[0] = IMX334_VMAX_8M_30FPS_12BIT_LINEAR;
    g_apstSnsState[ViPipe]->au32FL[1] = IMX334_VMAX_8M_30FPS_12BIT_LINEAR;
...
static int imx334_set_bus_info(VI_PIPE ViPipe, ISP_SNS_COMMBUS_U unSNSBusInfo)
{
