...
HI_VOID ISP_RegsDefault(VI_PIPE ViPipe)
{
    ...
    {
        isp_clip_y_min_write(ViPipe, i , ISP_CLIP_Y_MIN_DEFAULT);
        isp_clip_y_max_write(ViPipe, i , ISP_CLIP_Y_MAX_DEFAULT);
        isp_clip_c_min_write(ViPipe, i , ISP_CLIP_C_MIN_DEFAULT);
        isp_clip_c_max_write(ViPipe, i , ISP_CLIP_C_MAX_DEFAULT);
        isp_csc_sum_en_write(ViPipe, i , ISP_CSC_SUM_EN_DEFAULT);
        isp_yuv444_sum_en_write(ViPipe, i , ISP_YUV444_SUM_EN_DEFAULT);
        isp_yuv422_sum_en_write(ViPipe, i , ISP_YUV422_SUM_EN_DEFAULT);
        isp_wdr_sum_en_write(ViPipe, i , ISP_WDR_SUM_EN_DEFAULT);
        isp_demosaic_sum_en_write(ViPipe, i , ISP_DEMOSAIC_SUM_EN_DEFAULT);
    }
...
static HI_VOID ISP_ProNrParamInitialize(VI_PIPE ViPipe)
{
    ...
    for (k = 0; k < PRO_MAX_FRAME_NUM; k++)
    {
        for (i = 0; i < ISP_AUTO_ISO_STRENGTH_NUM; i++)
        {
            ...
            pstIspCtx->stProNrParamCtrl.pstProNrParam->astNrAttr[k].au8ChromaStr[0][i]  = HI_EXT_SYSTEM_BAYERNR_MANU_CHROMA_STRENGTH_DEFAULT;
            pstIspCtx->stProNrParamCtrl.pstProNrParam->astNrAttr[k].au8ChromaStr[1][i]  = HI_EXT_SYSTEM_BAYERNR_MANU_CHROMA_STRENGTH_DEFAULT;
            pstIspCtx->stProNrParamCtrl.pstProNrParam->astNrAttr[k].au8ChromaStr[2][i]  = HI_EXT_SYSTEM_BAYERNR_MANU_CHROMA_STRENGTH_DEFAULT;
            pstIspCtx->stProNrParamCtrl.pstProNrParam->astNrAttr[k].au8ChromaStr[3][i]  = HI_EXT_SYSTEM_BAYERNR_MANU_CHROMA_STRENGTH_DEFAULT;
            pstIspCtx->stProNrParamCtrl.pstProNrParam->astNrAttr[k].au16CoarseStr[0][i] = HI_EXT_SYSTEM_BAYERNR_MANU_COARSE_STRENGTH_DEFAULT;
            pstIspCtx->stProNrParamCtrl.pstProNrParam->astNrAttr[k].au16CoarseStr[1][i] = HI_EXT_SYSTEM_BAYERNR_MANU_COARSE_STRENGTH_DEFAULT;
            pstIspCtx->stProNrParamCtrl.pstProNrParam->astNrAttr[k].au16CoarseStr[2][i] = HI_EXT_SYSTEM_BAYERNR_MANU_COARSE_STRENGTH_DEFAULT;
            pstIspCtx->stProNrParamCtrl.pstProNrParam->astNrAttr[k].au16CoarseStr[3][i] = HI_EXT_SYSTEM_BAYERNR_MANU_COARSE_STRENGTH_DEFAULT;
        }
    }
}
