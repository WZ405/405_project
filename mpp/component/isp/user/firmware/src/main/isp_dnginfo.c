...
HI_S32 ISP_DngInfoInit(VI_PIPE ViPipe, ISP_DNG_INFO_CTRL_S *pstDngInfoCtrl)
{
    HI_S32 s32Ret;

    s32Ret = ioctl(g_as32IspFd[ViPipe], ISP_DNG_INFO_BUF_INIT, &pstDngInfoCtrl->u64DngInfoPhyaddr);
    ...
    pstDngInfoCtrl->pstIspDng = HI_MPI_SYS_Mmap(pstDngInfoCtrl->u64DngInfoPhyaddr, sizeof(DNG_IMAGE_STATIC_INFO_S));
    if (HI_NULL == pstDngInfoCtrl->pstIspDng)
    {
        printf("mmap Dng info buf failed!\n");
        s32Ret = ioctl(g_as32IspFd[ViPipe], ISP_DNG_INFO_BUF_EXIT);
        if (HI_SUCCESS != s32Ret)
        {
...
HI_S32 ISP_DngInfoExit(VI_PIPE ViPipe, ISP_DNG_INFO_CTRL_S *pstDngInfoCtrl)
{
    HI_S32 s32Ret;
...
HI_S32 ISP_DngColorParamUpdate(VI_PIPE ViPipe)
{
    ...
    AWB_CCM_CONFIG_S stAwbCcmConfig;
    ISP_DNG_CCM_S stDngCcm;
    HI_DOUBLE adHiD50Ccm[9], adHiACcm[9];
    HI_DOUBLE adHiD50Wbgain[9], adHiAWbgain[9];
    HI_U16 au16HiD50Wbgain[9], au16HiAWbgain[9];
    HI_DOUBLE adAMultMatrix[9], adD50MultMatrix[9];
    HI_DOUBLE adInvAColorMatrix[9], adInvD50ColorMatrix[9];
    HI_DOUBLE adAColorMatrix[9], adD50ColorMatrix[9];
    HI_DOUBLE adAForwardMatrix[9], adD50ForwardMatrix[9];
    ISP_DNG_COLORPARAM_S stDngColorParam;
    HI_U8 i;
...
HI_S32 ISP_UpdateDngImageDynamicInfo(VI_PIPE ViPipe)
{
    ...
    {
        stDngImageDynamicInfo.au32BlackLevel[0] = hi_ext_system_black_level_00_read(ViPipe);
        stDngImageDynamicInfo.au32BlackLevel[1] = hi_ext_system_black_level_01_read(ViPipe);
        stDngImageDynamicInfo.au32BlackLevel[2] = hi_ext_system_black_level_10_read(ViPipe);
        stDngImageDynamicInfo.au32BlackLevel[3] = hi_ext_system_black_level_11_read(ViPipe);
    }
