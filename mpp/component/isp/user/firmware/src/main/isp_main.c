...
HI_S32 ISP_UpdateAttachInfo(VI_PIPE ViPipe)
{
    ...
    if (!pstIspCtx->stAttachInfoCtrl.bSupportAttach)
    {
        return HI_FAILURE;
    }
...
HI_S32 ISP_UpdateColorGamutinfo(VI_PIPE ViPipe)
{
    ...
    if (!pstIspCtx->stColorGamutInfoCtrl.bSupportColorGamut)
    {
        return HI_FAILURE;
    }

    if (HI_NULL == pstIspCtx->stColorGamutInfoCtrl.pstColorGamutInfo)
    {
    ...
    for (i = 0; i < ISP_MAX_ALGS_NUM; i++)
    {
        if (pstIspCtx->astAlgs[i].bUsed)
        {
            if (HI_NULL != pstIspCtx->astAlgs[i].stAlgFunc.pfn_alg_ctrl)
            {
                pstIspCtx->astAlgs[i].stAlgFunc.pfn_alg_ctrl(ViPipe, ISP_COLORGAMUTINFO_GET, pstIspCtx->stColorGamutInfoCtrl.pstColorGamutInfo);
            }
        }
    }
...
HI_S32 ISP_SwitchImageMode(VI_PIPE ViPipe)
{
    ...
    if (0 == pstIspCtx->u8IspImageModeFlag)
    {
        ...
        pstIspCtx->stPreSnsImageMode.u8SnsMode = pstIspCtx->stSnsImageMode.u8SnsMode;
    ...
    else
    {
        ...
        {
            s32Ret1 = ISP_BlockUpdate(ViPipe, &pstIspCtx->stBlockAttr);
            ...
            s32Ret1 = ISP_GetBeLastBuf(ViPipe);
            if (HI_SUCCESS != s32Ret1)
            {
            ...
            s32Ret1 = ioctl(g_as32IspFd[ViPipe], ISP_GET_PIPE_SIZE, &pstIspCtx->stPipeSize);
            if (HI_SUCCESS != s32Ret1)
            {
            ...
            s32Ret1 = ISP_SensorSetImageMode(ViPipe, &stSnsImageMode);
            ...
            if (HI_SUCCESS == s32Ret1)   /* need to init sensor */
            {
                ...
                s32Ret3 = ioctl(g_as32IspFd[ViPipe], ISP_RES_SWITCH_SET, &stResSwitchMode);
                ...
                s32Ret2 = ISP_SwitchResRegSet(ViPipe);
                ...
                s32Ret2 = ISP_SensorSwitch(ViPipe);
...
HI_S32 ISP_SwitchWDR(VI_PIPE ViPipe)
{
    ...
    /* 8. get new sensor default param */
    ISP_SensorGetDefault(ViPipe, &pstSnsDft);
...
HI_S32 ISP_SnapAttrGet(VI_PIPE ViPipe)
{
    ...
    if (HI_SUCCESS != s32Ret)
    {
        //printf("Get snap attr failed with ec %#x!\n", s32Ret);
        return s32Ret;
    }
...
HI_S32 ISP_SnapPreProcess(VI_PIPE ViPipe)
{
    ...
    if (pstIspCtx->stLinkage.s32PreviewPipeId > -1)
    {
