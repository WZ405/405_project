...
HI_S32 ISP_GetLightboxGain(VI_PIPE ViPipe, ISP_AWB_Calibration_Gain_S *pstAWBCalibrationGain)
{
    ...
    pstWBStat = (ISP_WB_STATISTICS_S *)malloc(sizeof(ISP_WB_STATISTICS_S));
    if (pstWBStat == HI_NULL)
    {
        printf("%s: LINE: %d pstWBStat malloc failure !\n", __FUNCTION__, __LINE__);
        return HI_FAILURE;
    }
    pstStatCfg = (ISP_STATISTICS_CFG_S *)malloc(sizeof(ISP_STATISTICS_CFG_S));
    if (pstStatCfg == NULL)
    {
        printf("%s: LINE: %d pstWBStat malloc failure !\n", __FUNCTION__, __LINE__);
        if (pstWBStat)
    ...
    HI_MPI_ISP_GetStatisticsConfig(ViPipe, pstStatCfg);
    if (pstStatCfg->stWBCfg.u16ZoneCol * pstStatCfg->stWBCfg.u16ZoneRow < 16)
    {
        printf("%s: LINE: %d Not support zone number less than 16 !\n", __FUNCTION__, __LINE__);
        if (pstWBStat)
        {
            free(pstWBStat);
        }
    ...
    {
        printf("%s: LINE: %d Get WB statics failed!\n", __FUNCTION__, __LINE__);
        if (pstWBStat)
        {
            free(pstWBStat);
        }
