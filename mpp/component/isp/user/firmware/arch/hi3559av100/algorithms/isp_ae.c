...
static HI_S32 AeGetFrameInfo(VI_PIPE ViPipe, ISP_AE_RESULT_S *pstAeResult)
{
    ...
    //for(i=0; i<4; i++)
    {
        pstIspFrame->u32ExposureTime = pstAeResult->u32IntTime[0];
    ...
    {
        pstIspFrame->au32Ratio[0] = (pstAeResult->u32IntTime[1] << 6) / DIV_0_TO_1(pstAeResult->u32IntTime[0]);
    ...
    {
        pstIspFrame->au32Ratio[0] = (pstAeResult->u32IntTime[1] << 6) / DIV_0_TO_1(pstAeResult->u32IntTime[0]);
        pstIspFrame->au32Ratio[1] = (pstAeResult->u32IntTime[2] << 6) / DIV_0_TO_1(pstAeResult->u32IntTime[1]);
        pstIspFrame->au32Ratio[2] = 64;
    }
    else if (IS_4to1_WDR_MODE(pstIspCtx->u8SnsWDRMode))
    {
        pstIspFrame->au32Ratio[0] = (pstAeResult->u32IntTime[1] << 6) / DIV_0_TO_1(pstAeResult->u32IntTime[0]);
        pstIspFrame->au32Ratio[1] = (pstAeResult->u32IntTime[2] << 6) / DIV_0_TO_1(pstAeResult->u32IntTime[1]);
        pstIspFrame->au32Ratio[2] = (pstAeResult->u32IntTime[3] << 6) / DIV_0_TO_1(pstAeResult->u32IntTime[2]);
    }
    else
    {
...
static HI_VOID AeResRegsDefault(VI_PIPE ViPipe, ISP_REG_CFG_S *pstRegCfg)
{
    ...
    for (i = 0; i < u8BlockNum; i++)
    {
        ...
        pstAeStaticRegCfg->u16BEWidth  = stBlockRect.u32Width;
        pstAeStaticRegCfg->u16BEHeight = stBlockRect.u32Height;
        pstMgStaticRegCfg->u16Width    = stBlockRect.u32Width;
        pstMgStaticRegCfg->u16Height   = stBlockRect.u32Height;
        ...
        if (i == 0)
        {
            if (u8BlockNum > 1)
            {
                ...
                pstAeStaticRegCfg->u16BECropOutHeight = pstAeStaticRegCfg->u16BEHeight;
                pstAeStaticRegCfg->u16BECropOutWidth  = pstAeStaticRegCfg->u16BEWidth - u16Overlap;

                pstMgStaticRegCfg->u16CropPosX = 0;
                pstMgStaticRegCfg->u16CropPosY = 0;
                pstMgStaticRegCfg->u16CropOutHeight = pstMgStaticRegCfg->u16Height;
                pstMgStaticRegCfg->u16CropOutWidth  = pstMgStaticRegCfg->u16Width - u16Overlap;
            }
            else
            {
                ...
                pstAeStaticRegCfg->u16BECropPosX = 0;
                pstAeStaticRegCfg->u16BECropPosY = 0;
                pstAeStaticRegCfg->u16BECropOutHeight = u16Height;
                pstAeStaticRegCfg->u16BECropOutWidth  = u16Width;

                pstMgStaticRegCfg->u16CropPosX = 0;
                pstMgStaticRegCfg->u16CropPosY = 0;
                pstMgStaticRegCfg->u16CropOutHeight = u16Height;
                pstMgStaticRegCfg->u16CropOutWidth  = u16Width;
            }
        }
        ...
        else
        {
            ...
            pstAeStaticRegCfg->u16BECropPosY = 0;
            pstAeStaticRegCfg->u16BECropOutHeight = pstAeStaticRegCfg->u16BEHeight;
            pstAeStaticRegCfg->u16BECropOutWidth  = pstAeStaticRegCfg->u16BEWidth - (u16Overlap << 1);

            pstMgStaticRegCfg->u16CropPosX = u16Overlap;
            pstMgStaticRegCfg->u16CropPosY = 0;
            pstMgStaticRegCfg->u16CropOutHeight = pstMgStaticRegCfg->u16Height;
            pstMgStaticRegCfg->u16CropOutWidth = pstMgStaticRegCfg->u16Width - (u16Overlap << 1);
        }
    }
    ...
    pstAeStaticRegCfg->u16FECropOutHeight = u16Height;
    pstAeStaticRegCfg->u16FEHeight        = u16Height;
    pstAeStaticRegCfg->u16FEWidth         = u16Width;
    pstAeStaticRegCfg->u16FECropOutWidth  = u16Width;
...
static HI_VOID AeRegsDefault(VI_PIPE ViPipe, ISP_REG_CFG_S *pstRegCfg)
{
    ...
    for (i = 0; i < u8BlockNum; i++)
    {
        ...
        pstAeStaticRegCfg->u8BEBLCEnable = HI_FALSE;
        pstAeStaticRegCfg->u16BEOffsetR  = (pstSnsBlackLevel->au16BlackLevel[0] << 2);
        pstAeStaticRegCfg->u16BEOffsetGB = (pstSnsBlackLevel->au16BlackLevel[1] << 2);
        pstAeStaticRegCfg->u16BEOffsetGR = (pstSnsBlackLevel->au16BlackLevel[2] << 2);
        pstAeStaticRegCfg->u16BEOffsetB  = (pstSnsBlackLevel->au16BlackLevel[3] << 2);

        pstMgStaticRegCfg->u8Enable    = HI_TRUE;
        pstMgStaticRegCfg->u8BLCEnable = HI_FALSE;
        pstMgStaticRegCfg->u16OffsetR  = (pstSnsBlackLevel->au16BlackLevel[0] << 2);
        pstMgStaticRegCfg->u16OffsetGB = (pstSnsBlackLevel->au16BlackLevel[1] << 2);
        pstMgStaticRegCfg->u16OffsetGR = (pstSnsBlackLevel->au16BlackLevel[2] << 2);
        pstMgStaticRegCfg->u16OffsetB  = (pstSnsBlackLevel->au16BlackLevel[3] << 2);
    ...
    pstAeStaticRegCfg->u8FEBLCEnable = HI_FALSE;
    pstAeStaticRegCfg->u16FEOffsetR  = (pstSnsBlackLevel->au16BlackLevel[0] << 2);
    pstAeStaticRegCfg->u16FEOffsetGB = (pstSnsBlackLevel->au16BlackLevel[1] << 2);
    pstAeStaticRegCfg->u16FEOffsetGR = (pstSnsBlackLevel->au16BlackLevel[2] << 2);
    pstAeStaticRegCfg->u16FEOffsetB  = (pstSnsBlackLevel->au16BlackLevel[3] << 2);
    ...
    for (j = 0; j < WEIGHT_TABLE_HEIGHT; j++)
    {
        ...
        {
            pstAeDynaRegCfg->au8FEWeightTable[j][k] = au8WeightTable[j][k];
        }
    }
...
static HI_VOID AeUpdateConfig(VI_PIPE ViPipe, ISP_AE_RESULT_S *pstAeResult, ISP_REG_CFG_S *pstRegCfg)
{
    ...
    pstAeStatRegCfg = &pstRegCfg->stAlgRegCfg[0].stAeRegCfg.stStaticRegCfg;
    pstMgStatRegCfg = &pstRegCfg->stAlgRegCfg[0].stMgRegCfg.stStaticRegCfg;
...
static HI_VOID AeUpdateLinkage(VI_PIPE ViPipe, ISP_AE_RESULT_S *pstAeResult, ISP_LINKAGE_S *pstLinkage)
{
    ...
    pstLinkage->u32Inttime = pstAeResult->u32IntTime[0];
    pstLinkage->u8AERunInterval = pstAeResult->u8AERunInterval;

    if (IS_2to1_WDR_MODE(pstIspCtx->u8SnsWDRMode))
    {
        /* WDR exposure ratio is 6bit precision */
        pstLinkage->u32Inttime = pstAeResult->u32IntTime[1];
        pstLinkage->u32ExpRatio = (pstAeResult->u32IntTime[1] << 6) / DIV_0_TO_1(pstAeResult->u32IntTime[0]);
        pstLinkage->au32ExpRatio[0] = (pstAeResult->u32IntTime[1] << 6) / DIV_0_TO_1(pstAeResult->u32IntTime[0]);
    ...
    {
        pstLinkage->u32Inttime = pstAeResult->u32IntTime[2];
        pstLinkage->u32ExpRatio = (pstAeResult->u32IntTime[2] << 6) / DIV_0_TO_1(pstAeResult->u32IntTime[0]);
        pstLinkage->au32ExpRatio[0] = (pstAeResult->u32IntTime[1] << 6) / DIV_0_TO_1(pstAeResult->u32IntTime[0]);
        pstLinkage->au32ExpRatio[1] = (pstAeResult->u32IntTime[2] << 6) / DIV_0_TO_1(pstAeResult->u32IntTime[1]);
        hi_ext_system_actual_wdr_exposure_ratio_write(ViPipe, 0, pstLinkage->au32ExpRatio[0]);
    ...
    {
        pstLinkage->u32ExpRatio = (pstAeResult->u32IntTime[3] << 6) / DIV_0_TO_1(pstAeResult->u32IntTime[0]);
        pstLinkage->au32ExpRatio[0] = (pstAeResult->u32IntTime[1] << 6) / DIV_0_TO_1(pstAeResult->u32IntTime[0]);
        pstLinkage->au32ExpRatio[1] = (pstAeResult->u32IntTime[2] << 6) / DIV_0_TO_1(pstAeResult->u32IntTime[1]);
        pstLinkage->au32ExpRatio[2] = (pstAeResult->u32IntTime[3] << 6) / DIV_0_TO_1(pstAeResult->u32IntTime[2]);
        hi_ext_system_actual_wdr_exposure_ratio_write(ViPipe, 0, pstLinkage->au32ExpRatio[0]);
        hi_ext_system_actual_wdr_exposure_ratio_write(ViPipe, 1, pstLinkage->au32ExpRatio[1]);
                 ...
                 HI_VOID *pRegCfg, HI_S32 s32Rsv)
{
    ...
    u16BlackOffset = hi_ext_system_black_level_00_read(ViPipe);
    pstLib->stAeRegsiter.stAeExpFunc.pfn_ae_ctrl(pstLib->stAlgLib.s32Id,
...
HI_S32 ISP_AeCtrl(VI_PIPE ViPipe, HI_U32 u32Cmd, HI_VOID *pValue)
{
    ...
    {
        AeResRegsDefault(ViPipe, &pstRegCfg->stRegCfg);
    }
