...
static HI_VOID EdgeMarkRegsInitialize(VI_PIPE ViPipe, ISP_REG_CFG_S *pRegCfg)
{
    ...
    {
        pRegCfg->stAlgRegCfg[i].stEdgeMarkRegCfg.bEnable = HI_FALSE;
        EdgeMarkRegInit(&(pRegCfg->stAlgRegCfg[i].stEdgeMarkRegCfg));
    }
...
static HI_VOID EdgeMarkInitialize(VI_PIPE ViPipe)
{
    ISP_EDGEMARK_S     *pstEdgeMark = HI_NULL;
    ...
    if (pstSnsDft->stIspEdgeMark.bValid)
    {
        pstEdgeMark->bEnable     = pstSnsDft->stIspEdgeMark.bEnable;
        pstEdgeMark->u8Threshold = pstSnsDft->stIspEdgeMark.u8Threshold;
        pstEdgeMark->u32Color    = pstSnsDft->stIspEdgeMark.u32Color;
    }
    else
    {
...
static void EdgeMarkRgb2Uv(HI_U8 u8R, HI_U8 u8G, HI_U8 u8B,  HI_U16 *u,  HI_U16 *v)
{
    ...
    *u = (HI_U16)(CLIP3( ((cscCoef[0] * u8R + cscCoef[1] * u8G + cscCoef[2] * u8B + (128 << 10)) >> 10), 0, 255));
    *v = (HI_U16)(CLIP3(((cscCoef[3] * u8R + cscCoef[4] * u8G + cscCoef[5] * u8B + (128 << 10)) >> 10), 0, 255 ));
    *u = *u << 2;
    *v = *v << 2;
...
HI_S32 ISP_EdgeMarkExit(VI_PIPE ViPipe)
{
    ...
    {
        isp_sharpen_benmarkedge_write(ViPipe, i, HI_FALSE);
    }
