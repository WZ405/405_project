...
ISP_FLICKER_ATTR g_astFlickerCtx[ISP_MAX_PIPE_NUM];
...
static HI_VOID FlickerReadHWStatInfo(VI_PIPE ViPipe)
{
    HI_U32 i;
    ...
    s32Val = isp_flick_gr_diff_cur_read(ViPipe, 0);//Diff
    s32Val = (s32Val & 0x400000) ? (0xff800000 | s32Val) : s32Val;
    pstFlickerCtx->stFckStatInfo.s32CurFrameDiffSumGr = s32Val;

    s32Val = isp_flick_gb_diff_cur_read(ViPipe, 0);//Diff
    s32Val = (s32Val & 0x400000) ? (0xff800000 | s32Val) : s32Val;
    pstFlickerCtx->stFckStatInfo.s32CurFrameDiffSumGb = s32Val;

    pstFlickerCtx->stFckStatInfo.u32CurFrameAbsSumGr = isp_flick_gr_abs_cur_read(ViPipe, 0);//ABS
    pstFlickerCtx->stFckStatInfo.u32CurFrameAbsSumGb = isp_flick_gb_abs_cur_read(ViPipe, 0);//ABS

    pstFlickerCtx->stFckStatInfo.u32CurFrameCountOver = isp_flick_count_over_cur_read(ViPipe, 0); //ABS

    pstFlickerCtx->s32CurFrameAvgGr = (2 * pstFlickerCtx->stFckStatInfo.s32CurFrameDiffSumGr + (pstFlickerCtx->u16Height >> 1)) / DIV_0_TO_1((HI_S32)pstFlickerCtx->u16Height);
    pstFlickerCtx->s32CurFrameAvgGb = (2 * pstFlickerCtx->stFckStatInfo.s32CurFrameDiffSumGb + (pstFlickerCtx->u16Height >> 1)) / DIV_0_TO_1((HI_S32)pstFlickerCtx->u16Height);

    isp_flick_gmean_raddr_write(ViPipe, 0, 0);

    for (i = 0; i < pstFlickerCtx->u16FckInx ; i++)
    {
        s32Val = (HI_S32)isp_flick_gmean_rdata_read(ViPipe, 0);
